<template>
  <div>
    <table class="table" v-if="alarms.pending && alarms.pending.length !== 0">
      <tbody class="table__body">
        <tr class="table__row" v-for="(alarm, index) in alarms.pending" :key="index">
          <th>{{ index + 1 }}</th>
...
export default {
  ...
  setup(props, context) {
    const alarms = ref();
    watchEffect(() => alarms.value = context.attrs.alarmData)
